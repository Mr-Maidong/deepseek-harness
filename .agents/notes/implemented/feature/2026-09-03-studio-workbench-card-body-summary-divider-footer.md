...
- **Expand/collapse**: the footer's right corner holds an icon toggle (the `expand.webp`/`collapse.webp` assets, the same disclosure affordance the file tree uses) that collapses the card body (`data-collapsed` hides it) and expands it back. The button keeps `aria-expanded` and a localized `aria-label`/`title`. Completed todos default to collapsed and uncompleted ones to expanded; the collapsed/expanded sets are component-local state keyed by todo id.
