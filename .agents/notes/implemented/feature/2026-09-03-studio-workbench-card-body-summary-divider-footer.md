...
- **Expand/collapse**: the footer's right corner holds an icon toggle on top of the file tree's disclosure chevron — a single right-arrow glyph (the `expand.webp` asset) that rotates 90deg to point down when expanded, animated with the file tree's 220ms ease-out-curve transition. Collapsing the body animates the `max-height`, padding, and top border down to zero instead of toggling `display`. The button keeps `aria-expanded` (drives the arrow rotation) and a localized `aria-label`/`title`. Completed todos default to collapsed and uncompleted ones to expanded; the collapsed/expanded sets are component-local state keyed by todo id.
