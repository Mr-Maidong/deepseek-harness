...
- **展开/收起**：底栏右侧放一个复用文件树展开指示头的图标切换按钮——单张右箭头字形（`expand.webp` 资产），展开时旋转 90 度变为下箭头，采用与文件树相同的 220ms 缓出曲线动画。收起时内容区把 `max-height`、内边距与上边框动画收拢到 0，而非直接切 `display`。按钮保留 `aria-expanded`（驱动箭头旋转）与本地化 `aria-label`/`title`。完成的灵光默认收起、未完成的默认展开；收起/展开集合是按 todo id 键控的组件本地状态。
