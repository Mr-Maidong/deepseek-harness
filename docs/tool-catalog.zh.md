...
| `@deepseek-ai/dsh-tool-todo` | `todo_write`, `workbench_complete` | `ctx.tools`、`owning Agent session` | `tool/call`、`todo/write`、`tool/result` | - | todo_write 是会话所有的状态；UI 将最新的 todo/write 事件渲染为检查清单。`allowParallelInProgress` 是没有默认值的必填项，因此本目录明确选择 `true`，对应描述允许同时存在多个 `in_progress` 项。选择 `false` 的部署会获得同一工具，但描述会要求只能有 1 个活动任务。 |
...
```json
{
  ...
  "properties": {
    "todoId": {
      "type": "string",
      "description": "Stable todo id from the task prompt."
    },
    "summary": {
      "type": "string",
...
workbench_complete 的状态由会话记录中的 `studio/todo-complete` 事件承载；工作台 UI 将同一 todoId 的最新完成记录折叠进其任务池。
