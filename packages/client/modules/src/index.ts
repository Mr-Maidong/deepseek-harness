...
export class ClientModuleRegistry extends Service {
  ...
  private readonly serveBundle = (req: IncomingMessage, res: ServerResponse): void => {
    ...
    // Package-owned browser assets ride /plugins/<scoped-package>/assets/...
    // (the CSS url() rewriter emits these; the tsdown build emits the files
    // beneath the package's lib/assets). Any other under-/plugins path,
    // including unadvertised combinations and /plugins/events when the HMR
    // row is absent, is an unknown resource.
    ...
    if (assetStart >= 0) {
      const path = this.assetPath(
        pathname.slice(prefix.length, assetStart),
        pathname.slice(assetStart + assetMarker.length),
      )
      if (path === undefined) {
        res.writeHead(404)
        res.end()
      ...
      res.writeHead(200, {
        'content-type': ASSET_MIME[extname(path)] ?? 'application/octet-stream',
