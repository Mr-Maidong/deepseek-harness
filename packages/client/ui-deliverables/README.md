...
- **Directories have no destination** — chips open files through the chat file opener (Studio's universal preview card, or the right Sidebar fallback), which shows files only; the former native folder handoff is gone rather than replaced.
