...
function visibleEntries(
  ...
  // surface even while the toggle keeps the rest hidden. The picker selects
  // directories only, so file rows never render here.
  const isDirectory = (entry: DirectoryEntry): boolean => entry.kind === 'directory'
  const displayable = (entry: DirectoryEntry): boolean => isDirectory(entry)
    && (showHidden || !entry.hidden || needle.startsWith('.'))
  ...
  return entries.filter((entry) => {
    ...
    return isDirectory(entry) && (showHidden || !entry.hidden)
  })
}
