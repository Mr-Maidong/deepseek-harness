...
export class SlotRegistry extends Service {
  ...
  private resolveStore(
    ...
    if (record.scope === 'root') {
      ...
      // Workspace stores key by the owning workspace id (derived from the
      // session id) so sessions in one workspace share one record; session
      // stores key by the session id directly.
      key = record.storeScope === 'workspace' ? this.workspaceKey(scopeBinding.key) : scopeBinding.key
      this.bindStoreScope({ key, ctx: scopeBinding.ctx })
    }
