...
4. 调整灵光台卡片：标题行显示 checkbox 和任务标题，详情显示任务定义，完成后显示完成总结摘要。
...
右侧灵光台按项目记录待办。每条待办包含任务标题、详细说明、执行状态和完成总结。标题行使用 checkbox 切换状态，模型完成任务后将整体方案、实现路径、修改文件和验证结果写入该待办的完成总结中。项目和待办暂时属于会话内状态，后续可接入持久化存储。
