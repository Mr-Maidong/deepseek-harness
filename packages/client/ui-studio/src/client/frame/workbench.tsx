...
export function StudioWorkbench(props: StudioWorkbenchProps): React.ReactElement {
  ...
  const sendProject = async (): Promise<void> => {
    ...
    setError(undefined)
    for (const todo of activeProject.todos) actions.updateTodoStatus(todo.id, 'in_progress')
    try {
      await sendToChat(activeProject.todos.map(todo => '- ' + todo.title + '\n  ' + todo.detail).join('\n'))
    } catch {
      for (const todo of activeProject.todos) actions.updateTodoStatus(todo.id, 'blocked')
      setError(t('workbench.sendFailed'))
    }
  }
  ...
  return <section className={css.workbench} aria-labelledby="studio-workbench-title">
    ...
    <div className={css.projectBar}>
      ...
      <div className={css.projectActions}>
        ...
        <button className={css.quietButton} type="button" disabled={activeProject?.todos.length === 0} aria-label={t('workbench.sendAll')} title={t('workbench.sendAll')} onClick={() => { void sendProject() }}><span className={css.sendIcon} aria-hidden="true" /></button>
