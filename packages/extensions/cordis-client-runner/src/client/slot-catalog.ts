...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'conversation.session.header\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-studio HeaderSearch id \'studio-header-search\'',
  ...
  {
    key: 'studio.center.editor',
    kind: 'single',
    scope: 'root',
    summary: 'Editor registrants present a workspace file above the conversation column.',
    doc: 'Editor registrants present a workspace file above the conversation column.',
    registerOptions: [],
    ownerProps: [
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'root\' (client-ui-studio), so it exists while that entry is mounted',
    occupants: [
      'client-ui-studio PreviewCard',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'studio.center.editor\', () => ctx.slots.register(\n      { name: \'studio.center.editor\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-studio/src/client/index.ts:83',
  },
  {
    key: 'studio.center.toolbar',
    kind: 'single',
    scope: 'session',
    summary: 'Toolbar registrants replace the empty session-scoped toolbar seat.',
    doc: 'Toolbar registrants replace the empty session-scoped toolbar seat.',
    registerOptions: [],
    ownerProps: [
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'studio.center.toolbar\', () => ctx.slots.register(\n      { name: \'studio.center.toolbar\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-studio/src/client/index.ts:85',
  },
  {
    key: 'studio.left.main',
    kind: 'single',
    scope: 'root',
    summary: 'Left-main registrants remain available for compatibility with existing compositions.',
    doc: 'Left-main registrants remain available for compatibility with existing compositions.',
    registerOptions: [],
    ownerProps: [
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'studio.left.main\', () => ctx.slots.register(\n      { name: \'studio.left.main\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-studio/src/client/index.ts:75',
  },
  {
    key: 'studio.navigation',
    kind: 'single',
    scope: 'root',
    summary: 'Navigation registrants receive the active section and selection callback; absence renders Studio\'s built-in navigation.',
    doc: 'Navigation registrants receive the active section and selection callback; absence renders Studio\'s built-in navigation.',
    registerOptions: [],
    ownerProps: [
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'studio.navigation\', () => ctx.slots.register(\n      { name: \'studio.navigation\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-studio/src/client/index.ts:73',
  },
  {
    key: 'studio.status',
    kind: 'single',
    scope: 'root',
    summary: 'Status registrants receive the active section; absence leaves the status column empty.',
    doc: 'Status registrants receive the active section; absence leaves the status column empty.',
    registerOptions: [],
    ownerProps: [
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'studio.status\', () => ctx.slots.register(\n      { name: \'studio.status\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-studio/src/client/index.ts:79',
  },
  {
    key: 'studio.workbench',
    kind: 'single',
    scope: 'session',
    summary: 'Workbench registrants receive the active section; absence leaves the status column empty.',
    doc: 'Workbench registrants receive the active section; absence leaves the status column empty.',
    registerOptions: [],
    ownerProps: [
    ...
    declaredBy: 'an entry in \'root\' (client-ui-studio), so it exists while that entry is mounted',
    occupants: [
      'client-ui-studio StudioWorkbench',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'studio.workbench\', () => ctx.slots.register(\n      { name: \'studio.workbench\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-studio/src/client/index.ts:81',
  },
  {
    key: 'studio.workspace',
    kind: 'single',
    scope: 'root',
    summary: 'Workspace registrants receive the active section; absence leaves the workspace column empty.',
    doc: 'Workspace registrants receive the active section; absence leaves the workspace column empty.',
    registerOptions: [],
    ownerProps: [
    ...
    declaredBy: 'an entry in \'root\' (client-ui-studio), so it exists while that entry is mounted',
    occupants: [
      'client-ui-studio LeftPanelMain',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'studio.workspace\', () => ctx.slots.register(\n      { name: \'studio.workspace\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-studio/src/client/index.ts:77',
  },
  {
    key: 'studio.workspace.directoryFlow',
    kind: 'single',
    scope: 'root',
    summary: 'Studio\'s workspace add flow, occupied by the selected native or browse picker.',
    doc: 'Studio\'s workspace add flow, occupied by the selected native or browse picker.',
    registerOptions: [],
    ownerProps: [
      '/**\n * Owner share of the directory-flow holes: the complete conversation between\n * the trigger surface and the picking interaction. The occupant reads `open`\n * to run/render its interaction and reports exactly one outcome per open.\n */\nexport interface DirectoryFlowOwnerProps {\n  /** True while a picking interaction is requested; flipping back to false withdraws the request. */\n  open: boolean\n  /** True while the owner adopts a picked path (`createWorkspace` in flight); occupants disable their commit affordances. */\n  busy: boolean\n  /** The operator picked a directory (absolute host path); the owner adopts it. */\n  onPicked: (path: string) => void\n  /** The operator dismissed the interaction; the owner just closes the flow. */\n  onCancel: () => void\n  /** The interaction itself failed (chooser missing, listing denied); the owner shows its error surface. */\n  onError: (message: string) => void\n}',
    ],
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'studio.workspace\' (client-ui-studio), so it exists while that entry is mounted',
    occupants: [
      'client-ui-directory-picker-browse BrowseDirectoryFlow',
      'client-ui-directory-picker-native NativeDirectoryFlow',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'studio.workspace.directoryFlow\', () => ctx.slots.register(\n      { name: \'studio.workspace.directoryFlow\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
