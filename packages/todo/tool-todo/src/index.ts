...
import type { TodoItem, WorkbenchTodoCompletion } from './types.ts'
...
export function apply(ctx: Context, config: Config): void {
  ...
  // Lingguang completion fold: every `studio/todo-complete` event adds one
  // todoId-keyed completion to the record the workbench store folds on sight.
  ctx.sessionProjections.register<'studioTodoCompletions', Record<string, WorkbenchTodoCompletion> | null>({
